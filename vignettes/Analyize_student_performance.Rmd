---
title: "Analyze_student_performance"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Analyze_student_performance}
...
And these identified subgroups are
...
rsavs_res <- data.frame(fitted = fit2$fitted.values[-test_idx], 
                        residual = fit2$residuals[-test_idx], 
                        mu_new = mu_new, 
                        mu_id = RSI_Mu_to_ID(mu_new, unique(mu_new)), 
                        subgroup_residual = y_vec - mu_new - x_mat %*% beta_new, 
                        course = c(rep("por", nrow(por_df)), rep("mat", nrow(mat_df)))[-test_idx])

idx <- which(mu_new == unique(mu_new)[2])
ggplot(data = NULL, aes(fit2$fitted.values[-test_idx], fill = as.factor(rsavs_res$mu_id), color = as.factor(rsavs_res$mu_id))) + 
  geom_density(alpha = 0.2) +
  geom_vline(aes(xintercept = c(
    median((y_vec - rsavs_res$subgroup_residual)[idx]), 
    median((y_vec - rsavs_res$subgroup_residual)[-idx]))
    ), 
    color = c(median((y_vec - rsavs_res$subgroup_residual)[idx]), 
                 median((y_vec - rsavs_res$subgroup_residual)[-idx])), 
    linetype = "dashed"
    ) +
  xlab("Response") +
  labs(color = "Subgroup", fill = "Subgroup")
```
